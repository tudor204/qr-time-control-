...
export const ProductivityWidget: React.FC<ProductivityWidgetProps> = ({ employee, title, records }) => {
    ...
    return (
        <div className={`p-6 md:p-8 rounded-[2.5rem] shadow-sm border mb-6 relative overflow-hidden transition-all group ${stats.onVacation ? 'bg-orange-50 border-orange-200' : 'bg-white border-slate-100 hover:shadow-xl hover:shadow-blue-600/5'}`}>
            {/* Fondo Decorativo Sutil */}
            <div className={`absolute top-0 right-0 w-32 h-32 blur-3xl opacity-10 rounded-full -mr-16 -mt-16 transition-all duration-700 group-hover:scale-150 ${stats.onVacation ? 'bg-orange-500' : 'bg-blue-600'}`}></div>
            ...
            <div className="flex items-baseline gap-2 mb-6 relative z-10 transition-transform group-hover:translate-x-1 duration-300">
                <span className={`text-5xl font-black tracking-tighter ${stats.onVacation ? 'text-orange-600' : 'text-slate-900'}`}>
                    {stats.total}
                </span>
            ...
            <div className="relative z-10">
                <div className="flex justify-between text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest">
                    ...
                    <span className={stats.onVacation ? 'text-orange-500' : 'text-blue-600'}>{Math.round(stats.percent)}%</span>
                </div>
                <div className="w-full bg-slate-100 h-4 rounded-2xl p-1 overflow-hidden">
                    <div
                        className={`h-full rounded-xl transition-all duration-1000 ease-out relative ${stats.onVacation ? 'bg-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.3)]' : 'bg-blue-600 shadow-[0_0_15px_rgba(37,99,235,0.3)]'}`}
